...
class LogController extends \TYPO3\CMS\Extbase\Mvc\Controller\ActionController
{
    ...
    // 1. Fetch Page Views & Join with Pages table for titles
    ...
    // 2. Fetch Backend Activity & Join with Pages table
    ...
    foreach ($activityRows as $row) {
        ...
        // We calculate the first letter here in PHP to avoid the Fluid error
        $row['user_initial'] = strtoupper(substr($row['display_user'], 0, 1));
        $backendActivity[] = $row;
    }


        // --- Dynamic & Safe Site Resource Audit ---
        $stats = [
        ...
        foreach ($pathsToScan as $path) {
            ...
            foreach ($iterator as $fileInfo) {
                // FIX: Check if the file actually exists and is readable before getting size
                if ($fileInfo->isFile() && $fileInfo->isReadable()) {
                    try {
                        ...
                        // Skip files that throw errors (broken symlinks)
                        continue;
                    }
                }
            }
